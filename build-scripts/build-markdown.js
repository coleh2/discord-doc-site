...
const ROOT_URL = "https://doc.nhs.gg"
...
var sitemap = require(path.resolve(__dirname, "sitemap-builder.js"));
...
for(var i = 0; i < mdFiles.length; i++) {
    ...
    compileMarkdown(source, mdFiles[i], redirects[filePath].substring(0,shortenerLength), function(html,builtFileName, metadata) {
        ...
        if(iSave+1 == mdFiles.length) {
            ...
            let sitemapFile = path.join(mdBuildFolder, "sitemap.xml");
            ...
            fs.writeFileSync(sitemapFile, sitemap.toXml());
        }
    });
